  ...
  <div class="container">
    <div class="row mb-4">
      <div class="col-12 text-center">
        ...
        <p class="text-muted mb-3">Cổng thông tin nội bộ - Portal Nhân viên</p>
        <div class="d-flex justify-content-center gap-3">
          <a href="http://localhost/LTW/creative-agency-hub" class="btn btn-dark rounded-pill shadow-sm"><i
      ...
      <div class="col-lg-4">
        <div class="card card-custom h-100">
          <div class="card-body p-4">
            ...
            <form method="POST">
              <div class="mb-3"><label class="form-label fw-bold text-muted">Từ ngày:</label><input type="date"
                  name="start_date" class="form-control" required min="<?= date('Y-m-d') ?>"></div>
              <div class="mb-3"><label class="form-label fw-bold text-muted">Đến ngày:</label><input type="date"
                  name="end_date" class="form-control" required min="<?= date('Y-m-d') ?>"></div>
              <div class="mb-4"><label class="form-label fw-bold text-muted">Lý do nghỉ phép:</label><textarea
                  name="reason" class="form-control" rows="3" required placeholder="Nhập chi tiết lý do..."></textarea>
              </div>
              <button type="submit" name="submit_leave" class="btn btn-primary w-100 rounded-pill py-2 fw-bold"
                <?= $leave_balance <= 0 ? 'disabled' : '' ?>><i class="bi bi-send me-1"></i> GỬI YÊU CẦU</button>
            </form>
          </div>
        </div>
      </div>
            ...
            <div class="table-responsive">
              <table class="table table-hover align-middle">
                ...
                <tbody>
                  ...
                  <tr>
                    <td colspan="4" class="text-center text-muted py-4">Chưa có dữ liệu nghỉ phép.</td>
                  </tr>
                  <?php else: foreach($requests as $req): $d = (strtotime($req['end_date']) - strtotime($req['start_date'])) / 86400 + 1; ?>
                  <tr>
                    <td><strong><?= date('d/m/Y', strtotime($req['start_date'])) ?></strong> <br><small
                        class="text-muted">đến <?= date('d/m/Y', strtotime($req['end_date'])) ?></small></td>
                    <td><?= htmlspecialchars($req['reason']) ?></td>
                    <td class="text-center"><span class="badge bg-secondary"><?= $d ?> ngày</span></td>
                    <td class="text-center">
                      <?php if($req['status'] == 'Pending'): ?><span class="badge bg-warning text-dark rounded-pill"><i
                          class="bi bi-hourglass-split"></i> Chờ duyệt</span>
                      <?php elseif($req['status'] == 'Approved'): ?><span class="badge bg-success rounded-pill"><i
                          class="bi bi-check-circle"></i> Đã duyệt</span>
                      <?php else: ?><span class="badge bg-danger rounded-pill"><i class="bi bi-x-circle"></i> Từ
                        chối</span><?php endif; ?>
                    </td>
                  </tr>
