...
class TaskController {
    ...
    public function store() {
        ...
        if (!$assigner_id) {
            ...
            echo json_encode(["status" => "error", "message" => "Không xác định được danh tính người giao việc"]);
            exit;
        }
        ...
        $taskId = $this->taskModel->createTask(
            ...
            $input['deadline'], $assigner_id, $assignee_id,
        ...
        if ($taskId) {
            ...
            TaskActivityService::log(
                $taskId, $assigner_id, TaskAction::CREATE,
            ...
            if ($assignee_id) {
                ...
                // Log: giao việc
                TaskActivityService::log(
                    $taskId, $assigner_id, TaskAction::ASSIGN,
            ...
            if ($watcher_id) {
                NotificationService::send(
                    $watcher_id,
                    "Bạn được thêm vào vị trí theo dõi task: " . $input['title']
                );
            }
            ...
            echo json_encode(["status" => "error", "message" => "Lỗi server khi tạo Task"]);
        }
        exit;
    }
    ...
    public function update($taskId) {
        ...
        // Chỉ gọi getTaskById 1 lần
        $task = $this->taskModel->getTaskById($taskId);
        if (!$task) {
            http_response_code(404);
            echo json_encode(["status" => "error", "message" => "Không tìm thấy công việc"]);
            exit;
        }
        ...
        $success = $this->taskModel->updateTask(
            $taskId, $input['title'], $description, $priority,
            $input['deadline'], $assignee_id, $watcher_id, $project_id
        );
        ...
        if ($success) {
            ...
            TaskActivityService::log(
                $taskId, $user_id, TaskAction::UPDATE,
                "{$actor['full_name']} updated task \"{$input['title']}\""
            );

            NotificationService::sendToMany(
                array_filter([$task['assignee_id'], $task['watcher_id']]),
                "Task '{$input['title']}' vừa được cập nhật"
            );
            ...
            http_response_code(500);
            echo json_encode(["status" => "error", "message" => "Lỗi server khi cập nhật công việc"]);
        }
        exit;
    }
    ...
    public function updateStatus($taskId) {
        ...
        if (!isset($input['status'])) {
            http_response_code(400);
            echo json_encode(["status" => "error", "message" => "Thiếu dữ liệu status"]);
            exit;
        }
        ...
        if ($success) {
            ...
            TaskActivityService::log(
                $taskId, $user_id, TaskAction::STATUS_CHANGE,
                "{$actor['full_name']} moved task to \"{$input['status']}\""
            );

            // Chỉ dùng 1 hệ thống notification (NotificationService)
            NotificationService::sendToMany(
                array_filter([$task['assignee_id'], $task['watcher_id']]),
                "Task '{$task['title']}' đã chuyển trạng thái sang {$input['status']}"
            );
